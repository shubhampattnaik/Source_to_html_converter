...
void source_to_html(FILE* fp, pevent_t *event)
{
	...
	switch(event->type)
	{
		...
		case PEVENT_EOF ://puts(event->data);
			fprintf(fp,"%s",event->data);
			break;
		case PEVENT_NUMERIC_CONSTANT://puts(event->data);
			fprintf(fp,"<span class=\"numeric_constant\">%s</span>",event->data);
			break;
		case PEVENT_RESERVE_KEYWORD://puts(event->data);
